...
def test_format_dates():
	...
	dummy_test_date = pd.Series([pd.to_datetime("01/01/2025", format ="%d/%m/%Y"), pd.NaT])

	dummy_col = format_dates(dates_df['DMY'])

	pd.testing.assert_series_equal(dummy_col, dummy_test_date, check_names=False, check_dtype=False)
